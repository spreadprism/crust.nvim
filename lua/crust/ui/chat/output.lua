...
---
--- The conversation *is* the buffer: text is appended to it, tool calls are
--- runs of lines anchored by an extmark so they can be rewritten in place,
--- and highlights are extmarks on absolute rows. Nothing is virtualized,
--- so what you read here is what is on screen.
...
--- Write a whole conversation without following or re-rendering per message,
--- then do both once. Used when a session is replayed into the panel.
---@param fn fun()
...
--- 0-based row a block starts on, nil when it is gone.
---@param block Crust.Chat.Output.Block
---@return integer?
function Output:block_row(block)
...
function Output:block_at(row)
	...
	for _, block in ipairs(self._blocks) do
		local first = self:block_row(block)
		if first and row >= first and row < first + block.count then
			return block
		end
	end
	return nil
...
function Output:replace_block(block, lines, highlights, line_highlights)
	...
	vim.api.nvim_buf_clear_namespace(self._buf, hl_ns, row, row + block.count)

	vim.bo[self._buf].modifiable = true
	vim.api.nvim_buf_set_lines(self._buf, row, row + block.count, false, lines)
	-- Replacing the anchored line moves the extmark to the end of the new
	-- text, so re-anchor it on the first line of the block.
	vim.api.nvim_buf_set_extmark(self._buf, ns, row, 0, { id = block.id })
	vim.bo[self._buf].modifiable = false
	block.count = #lines
	self:_highlight_block(row, #lines, highlights, line_highlights)
	self:_refresh_regions()
...
function Output:_block_ranges()
	...
	for _, block in ipairs(self._blocks) do
		local row = self:block_row(block)
		if row then
			ranges[#ranges + 1] = { first = row, last = row + block.count }
		end
	end
	return ranges
end
...
function Output:_refresh_regions()
	if self._batch or not require("crust.config").get().raw_tool_blocks then
		return
	end
