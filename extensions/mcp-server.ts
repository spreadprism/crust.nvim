/**
 * Bundled with crust.nvim and passed to pi as `-e extensions/mcp-server.ts`
 * when `config.mcp.enabled` is true.
 *
 * It talks back to the neovim instance that spawned pi over the socket in
 * CRUST_NVIM_SERVER, calling `crust.integrations.mcp_server` functions with
...
export default function (pi: ExtensionAPI) {
	...
	async function remote(lua: string, signal?: AbortSignal): Promise<string | null> {
		const result = await pi.exec(
			"nvim",
			["--server", SERVER!, "--remote-expr", `luaeval("${lua.replace(/"/g, '\\"')}")`],
			{ signal, timeout: 5000 },
		);
	...
	pi.on("before_agent_start", async (_event, ctx) => {
		const snapshot = await remote("require('crust.integrations.mcp_server').snapshot()", ctx.signal);
		if (!snapshot) return;
	...
	pi.registerTool({
		...
		promptSnippet: "Inspect the user's open buffers and cursor position in neovim",
		promptGuidelines: [
			"Use nvim_context when the user refers to what they are looking at, 'this file', or 'here'.",
		],
		parameters: Type.Object({}),
		async execute(_toolCallId, _params, signal) {
			const snapshot = await remote("require('crust.integrations.mcp_server').snapshot()", signal);
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			path: Type.Optional(Type.String({ description: "File to report on, omit for every buffer" })),
		}),
		async execute(_toolCallId, params, signal) {
			const arg = params.path ? `'${params.path.replace(/'/g, "''")}'` : "";
			const output = await remote(
				`require('crust.integrations.mcp_server').diagnostics(${arg})`,
				signal,
			);
